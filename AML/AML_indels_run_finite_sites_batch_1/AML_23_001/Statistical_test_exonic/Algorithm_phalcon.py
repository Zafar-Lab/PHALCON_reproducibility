...
argParser.add_argument('-s', '--seed', type=int, default = 171215)
...
# Find best likelihoods among a set of chosen number of clusters
    ...
    cluster_labels = np.loadtxt('/home/priya/Downloads/Final_Stage/AML_results_finite_sites/AML_indels_run_finite_sites_batch_1/'+data+'/'+data+'_indels_finite_sites_inferred_cluster_labels.txt',dtype='float')
...
countFileName = '/home/priya/Downloads/Final_Stage/AML_Results/AML_indels_runs_batch_1/'+data+'/ReadCounts_'+data+'.tsv'
qualityFileName = '/home/priya/Downloads/Final_Stage/AML_Results/AML_indels_runs_batch_1/'+data+'/GQ_'+data+'.tsv'
outputPrefixName = data+'_sciphin_bf_test_'
seed=args.seed
...
inferredGenotypeFileName=outputPrefixName+'indels_inferred_genotypes.tsv'
...
df = df[df[1].isin([site_not_found_in_bulk,site_not_found_in_bulk-1])]
print(df)
df = df.to_numpy()
...
df_for_sites = pd.read_csv("/home/priya/Downloads/Final_Stage/AML_results_finite_sites/find_sites_not_present_in_bulk_for_statistical_testing/finite_sites_exonic_not_detected_in_bulk.csv")
...
if isQualityFilteringOn: 
    ...
    df_gq = df_gq[df_gq[0].isin([site_not_found_in_bulk,site_not_found_in_bulk-1])]
...
#print('End cluster : ',end_resolution)
...
best_lklhd = lklhd_s[inferred_index]
...
with open('Log BF test exonic.txt','a+') as f:
   f.write(str(data)+"\t"+str(site_not_found_in_bulk)+'\t'+str(best_lklhd)+'\t')
   

with open('Final clusters inferred.txt','w') as f:
   f.write('Final inferred clusters in the tree: '+str(final_inferred_clusters))
   f.write('\n')
   f.write("Final inferred distinct genotypes: " + str(best_mat_trimmed.shape[0]))
