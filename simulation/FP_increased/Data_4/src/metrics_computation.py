...
for i in mut_pos_obtained:
    if i not in mut_pos:
        l = [i]
        l.extend(list(np.zeros(df_orig.shape[1]-1)))
        df_orig.loc[len(df_orig.index)] = l


rslt_df = df_obtained.sort_values(by = 'index')
df_orig = df_orig.sort_values(by= df_orig.columns[0])
print(len(set(rslt_df['index']) & set(df_orig[0])))
