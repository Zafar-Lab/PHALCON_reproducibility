...
t.write(format=9, outfile="true_tree.nw")
...
cpn = ['None'] * int(total_pos)
for i in range(int(total_pos)):
  if random.uniform(0,1) < cpn_rate:
    cpn[i] = random.sample([1,3,4,5,6,7],1)[0] # select a true copy number out of 4,5,6 and 7
  else:
    cpn[i] = 2


path_for_cpn = "cpn_" + str(numCells) + "_" + str(int(m_pos)) + ".txt"
with open(path_for_cpn, 'w') as f:
  for i in range(int(total_pos)):
    f.write(str(i)+' ')
  f.write('\n')
  for i in range(int(total_pos)):
