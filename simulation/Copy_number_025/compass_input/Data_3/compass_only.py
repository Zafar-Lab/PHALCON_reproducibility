...
gv_modified_compass = '/home/priya/Downloads/Final_Stage/COMPASS/CPN_025/Data_'+data_no+'/Set 1/Compass_2000_'+no_clusters+'_'+data_no+'_tree_modified.gv'  
...
for i in range(len(new_content)):
  ...
  line4 = re.sub("\\n",'',new_content[i])
  new_content[i] = line4
  line5 = re.sub("<B>CNLOH Region0:REF,ALT </B>,<B>CNLOH RegionREF,ALT,REF </B>'",'',new_content[i])
  new_content[i] = line5
  line6 = re.sub("<B>CNLOH Region0:REF,ALT </B>,<B>CNLOH RegionALT,ALT,ALT </B>",'',new_content[i])
  new_content[i] = line6
  line7 = re.sub("<B>CNLOH Region0:REF",'',new_content[i])
  new_content[i] = line7
  line8 = re.sub("ALT </B>",'',new_content[i])
  new_content[i] = line8
  line9 = re.sub("<B>CNLOH RegionREF",'',new_content[i])
  new_content[i] = line9
  line10 = re.sub('ALT','',new_content[i])
  new_content[i] = line10
  line11 = re.sub("REF </B>",'',new_content[i])
  new_content[i] = line11
...
for i in new_content:
  if i==' ' or '':  # if there is no label, some instances empty string might also come into picture
    ...
    sites = i.split(',')
    sites = [int(site) for site in sites if site]
