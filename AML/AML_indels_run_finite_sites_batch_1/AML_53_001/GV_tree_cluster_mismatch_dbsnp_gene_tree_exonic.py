...
with open(data+'_indels_finite_sites_inferred_tree.nw','r') as f:
        string = f.read()
...
dbsnp_non_zero_avinput_file_name = '/home/priya/Downloads/Final_Stage/AML_results_finite_sites/AML_indels_run_finite_sites_batch_1/'+data+'/'+data+'_indels_dbsnp_non_zero_finite_sites.avinput.variant_function'
...
final_variants_df.loc[final_variants_df[1].str.contains('EZH2',case=False,na=False),1] = 'EZH2'
print("Exonic variants list : ",final_variants_df)
...
orig_pos = pd.read_csv(data+'_indels_finite_sites_final_df.tsv',sep='\t',header=None) # this is the dataframe which is obtained after post processing and everything
...
before_post_process_file = pd.read_csv(data+'_indels_finite_sites_final_df.tsv',sep='\t',header=None) # final data frame for finding the sites which are mutated
...
cluster_labels = list(np.loadtxt(data+'_indels_finite_sites_inferred_cluster_labels.txt',dtype='int')) # now finding the labels corresponding to each unique cell
columns = dict()
...
with open(data+'_indels_dbsnp_nz_finite_sites_inferred_tree_exonic.gv','w') as f:
  f.write('digraph G {\n')
  for node in tree_nw.traverse("preorder"):
...
with open(data+'_indels_dbsnp_nz_finite_sites_nonempty_inferred_tree_exonic.gv','w') as f:
  f.write('digraph G {\n')
  for node in tree_nw.traverse("preorder"):
    list_of_label = node.label
    delimiter = ','
...
def gene_tree_non_empty(data): # this function will convert all mutations named as "1:29997234" into their gene names
  gene_name = pd.read_csv(data+'_indels_dbsnp_non_zero_finite_sites.avinput.variant_function',sep='\t',header=None)
  with open(data+'_indels_dbsnp_nz_finite_sites_nonempty_inferred_tree_exonic.gv','r') as f:
     content = f.readlines()
  ...
  with open(data+'_indels_dbsnp_nz_finite_sites_nonempty_gene_tree_exonic.gv','w') as f:
    for i in range(len(content)):
      f.write(content[i])
  print("nw tree written as gv format")

  s = Source.from_file(data+'_indels_dbsnp_nz_finite_sites_nonempty_gene_tree_exonic.gv')
  s.render()


def gene_tree(data): # this function will convert all mutations named as "1:29997234" into their gene names
  gene_name = pd.read_csv(data+'_indels_dbsnp_non_zero_finite_sites.avinput.variant_function',sep='\t',header=None)
  with open(data+'_indels_dbsnp_nz_finite_sites_inferred_tree_exonic.gv','r') as f:
     content = f.readlines()
  gene_name = np.array(gene_name,dtype='object')
  gene_pos = dict()
  for i in range(gene_name.shape[0]):
  ...
  with open(data+'_indels_dbsnp_nz_finite_sites_gene_tree_exonic.gv','w') as f:
    for i in range(len(content)):
      f.write(content[i])
  print("nw tree written as gv format")

  s = Source.from_file(data+'_indels_dbsnp_nz_finite_sites_gene_tree_exonic.gv')
  s.render()
