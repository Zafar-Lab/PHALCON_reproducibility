...
tree_nw = Tree("((6,((7,(10,2)),(5,(4,(8,9))))),(3,(0,1)));")
data = 'TN4'
...
orig_pos = pd.read_csv(data+'_orig_hierarchical_final_df.tsv',sep=',',header=None) # this is the dataframe which is obtained after post processing and everything
config = pd.read_csv('/home/priya/Downloads/Final Stage/Real Data Results/'+data+'/Hierarchical_prior_FINAL/Genotype configuration.tsv',sep='\t',header=None) # Genotype configuration file
before_post_process_file = pd.read_csv('/home/priya/Downloads/Final Stage/Real Data Results/'+data+'/Hierarchical_prior_FINAL/'+data+'_orig_hierarchical_final_df.tsv',sep='\t',header=None) # final data frame for finding the sites which are mutated
...
cluster_labels = list(np.loadtxt('/home/priya/Downloads/Final Stage/Real Data Results/'+data+'/Hierarchical_prior_FINAL/'+data+'_orig_hierarchical_inferred_cluster_labels.txt',dtype='int')) # now finding the labels corresponding to each unique cell
columns = dict()
...
with open('Phalcon_non_empty_tree_'+str(data)+'.gv','w') as f:
  f.write('digraph G {\n')
  for node in tree_nw.traverse("preorder"):
...
with open('Phalcon_tree_'+str(data)+'.gv','w') as f:
  f.write('digraph G {\n')
  for node in tree_nw.traverse("preorder"):
    list_of_label = node.label
    delimiter = ','
