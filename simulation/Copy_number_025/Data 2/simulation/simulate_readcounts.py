...
argParser.add_argument('-D', '--dropoutRate', type=float, default = 0.2)   #dropout  : fn
...
argParser.add_argument('-s', '--seed', type=int, default = 179135) 
...
argParser.add_argument('-C','--copyNumberRate',type=float,default = 0.25)
...
rcFileName = prefixName +'readCounts_2'+ '.tsv'
numPos = args.numPos
...
def createReadCounts(cov, counts,isFp):  # the initial copies of wild type and alt type allele go here for polya urn
    newCounts = [0, 0, 0, 0]
    if cov==0:
        return newCounts
...
def writeMutType(rcFile, outFile, refNuc, mutNuc, alleleAffacted, cov,isMutPos,isFp,copy_number):
    nucs = ""
    counts = [0, 0, 0, 0]

    if alleleAffacted == '0':
        if copy_number ==2:
            counts[charToIndex(refNuc)] += alpha + beta  # no mutation, put all extra copies into refNuc
        else:
            counts[charToIndex(refNuc)] += copy_number

    elif alleleAffacted == '1': # heterozygous mutation
        if copy_number == 2:
            counts[charToIndex(refNuc)] += alpha
            counts[charToIndex(mutNuc)] += beta
        elif copy_number == 1:
            counts[charToIndex(mutNuc)] += copy_number
        else:   
            counts[charToIndex(refNuc)] += copy_number - 1  # initialising reference allele numbers as (copy number - 1)
            counts[charToIndex(mutNuc)] += 1  # remaining are alternate alleles
        
    elif alleleAffacted == '2':    # dropout case where alternate allele gets dropped out
        if copy_number == 2:
            counts[charToIndex(refNuc)] += alpha
        elif copy_number in [3,4,5,6]:
            # here, beta = 0   because there was only one alternate allele and it got dropped out
            counts[charToIndex(refNuc)] += copy_number - 1
        else:
            counts[charToIndex(refNuc)] += 1   # no drop outs here, only reference allele will be there


    elif alleleAffacted == '3':    # dropout case where reference allele gets dropped out
        if copy_number == 2:
            counts[charToIndex(mutNuc)] += beta
        elif copy_number in [3,4,5,6]:
            counts[charToIndex(refNuc)] += copy_number - 2  # even after one reference allele is dropped out, some reference alleles are left since its a copy number case
            counts[charToIndex(mutNuc)] += 1  # one alternate allele copy is still there
        else:
            counts[charToIndex(mutNuc)] += 1 # no drop outs here, only alternate allele will be there
...
def writeWildType(rcFile, outFile, refNuc, cov,isMutPos,isFp):
    writeMutType(rcFile, outFile, refNuc, refNuc, '0', cov,isMutPos,isFp, 2)
...
with loompy.connect("/home/priya/Downloads/Final_Stage/Loom Files/vr01.cells.loom") as ds:
...
for pos in tqdm(range (1, numPos + 1),desc="Creating Mpileup.."):
    ...
    for cell in range(0, numCells):
        ...
        if localCov == 0:
            ...
            if random.random()<=cpn:
                copy_number = random.choices(population = [1,3,4,5,6], weights = [0.0625, 0.5, 0.25, 0.125, 0.0625],k=1)[0]
                cpn_count += 1
            else:
                copy_number = 2
            
            if str(pos) in posMap: # out of mutated positions
                mutPosList.append(pos)
                if dropoutRate > 0:  # if drop-out rate is greater than 0
                    if random.uniform(0,1) < dropoutRate:   # and with probability of dropout rate
                        if random.uniform(0,1) < 1/copy_number:   # with less probability, alt allele is chosen and dropped out
                            posMap[str(pos)][cell] = '2' 
                        else:   # otherwise ref allele is chosen to be dropped out
                            posMap[str(pos)][cell] = '3' 
                copy_number_list.append([str(pos),copy_number])
                
                writeMutType(rcFile, outFile, ref, alt, posMap[str(pos)][cell], localCov,True,isFp,copy_number)
            else:
...
copy_number_arr = np.array(copy_number_list, dtype = 'int')
np.savetxt("Copy_numbers.txt", copy_number_arr.astype(int), fmt='%i', delimiter="\t")



with open('posMap.pkl', 'wb') as f:
    pickle.dump(posMap, f)
