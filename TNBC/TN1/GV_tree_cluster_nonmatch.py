...
print(numcells)
orig_pos = pd.read_csv('TN1_orig_hierarchical_final_df.tsv',sep='\t',header=None) # final data frame for finding the sites which are mutated
config.insert(loc = 0,column = 'chr', value = orig_pos[0])  # putting chromosome information at first column
config.insert(loc = 1,column = 'site',value = orig_pos[1]) # putting the site information at the second column
...
cluster_labels = list(np.loadtxt('TN1_orig_hierarchical_inferred_cluster_labels.txt',dtype='int')) # now finding the labels corresponding to each unique cell
columns = dict()
...
print("how many labels ",len(all_pos))
