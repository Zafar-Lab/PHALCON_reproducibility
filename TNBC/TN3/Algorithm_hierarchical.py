...
argParser.add_argument('-i', '--inReadCountFileName', type=str, default = '/home/priya/Downloads/Final Stage/Real Data Results/TN3/readCounts_MBTN3_nodups.tsv')
argParser.add_argument('-g', '--inQualityFileName', type=str,default = '/home/priya/Downloads/Final Stage/Real Data Results/TN3/gq_file_TN3.tsv')
argParser.add_argument('-o', '--outputPrefix', type=str,default='TN3_orig_hierarchical_')
...
argParser.add_argument('-e', '--eigenValueThreshold', type=float, default=0.05) 
...
argParser.add_argument('-s', '--seed', type=int, default = 110)
...
pd.DataFrame(one_Ls).to_csv("Ones_log_lklhd_matrix.tsv",sep='\t',index=False,header=False)
pd.DataFrame(zero_Ls).to_csv("Zeros_log_lklhd_matrix.tsv",sep='\t',index=False,header=False)
lklhd_s = []
best_mat_s = []
ysc_s = []
...
final_labels = np.empty(shape = (data_df.shape[0],1),dtype='int')
cluster_labels = np.array(y_sc)
cluster_dict = dict()
for i in set(cluster_labels):
    temp = [indices for indices in range(len(cluster_labels)) if cluster_labels[indices] == i]
    cluster_dict[i] = temp
final_labels = np.full((data_df.shape[0],),-1,dtype='int')
c=0
final_c = 0
for key in list(cluster_dict.keys()):
    ...
    max_sil_score = 0
    c += final_c
    colony = data_df[cluster_dict[key],:]
    if colony.shape[0] == 1534:
        colony_clusters_final = np.full((len(cluster_dict[key],)),-1,dtype='int')
        colony_adata = sc.AnnData(colony)
        for resol in [0.03]:
            print("Resolution :",resol)
            sc.pp.neighbors(colony_adata,use_rep = 'X')
            sc.tl.leiden(colony_adata,resolution=resol)
            colony_clusters = np.array(colony_adata.obs['leiden'],dtype='int')
            if len(set(colony_clusters)) ==1:
                sil_score = 2
            else:
                sil_score = silhouette_score(colony, colony_clusters)

            if sil_score > max_sil_score:
                max_sil_score = sil_score
                colony_clusters_final = colony_clusters
                final_c = len(set(colony_clusters_final))
                print("Clusters: ",len(set(colony_clusters_final)))
                print("Silhouette score :",sil_score)
    else:
       colony_clusters_final = np.full((len(cluster_dict[key],)),0,dtype='int')
       final_c = len(set(colony_clusters_final))
    for i in range(len(colony_clusters_final)):
        if key>0:
            colony_clusters_final[i] += c
    final_labels[cluster_dict[key]] = colony_clusters_final
...
print("best likelihood list shape :",len(lklhd_s))
print("List of matrices shape :",len(best_mat_s))
...
print("df before getrightorder :",df)
print("its shape :",df.shape)
print("gq shape before getrightorder :",gq.shape)
print("also gq :",gq)
