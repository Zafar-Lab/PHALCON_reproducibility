...
tree_nw = Tree("(((3,7),(2,5)),(6,(((0,1),(9,(4,10))),8)));")
data = 'TN5'
...
orig_pos = pd.read_csv(data+'_orig_spectral_final_df.tsv',sep=',',header=None) # this is the dataframe which is obtained after post processing and everything
config = pd.read_csv('/home/priya/Downloads/Final Stage/Real Data Results/'+data+'/Spectral_prior_FINAL/Genotype configuration.tsv',sep='\t',header=None) # Genotype configuration file
before_post_process_file = pd.read_csv('/home/priya/Downloads/Final Stage/Real Data Results/'+data+'/Spectral_prior_FINAL/'+data+'_orig_spectral_final_df.tsv',sep='\t',header=None) # final data frame for finding the sites which are mutated
...
cluster_labels = list(np.loadtxt('/home/priya/Downloads/Final Stage/Real Data Results/'+data+'/Spectral_prior_FINAL/'+data+'_orig_spectral_inferred_cluster_labels.txt',dtype='int')) # now finding the labels corresponding to each unique cell
columns = dict()
