# This file is to be used if the number of clusters is not equal to the distinct genotypes that are there in the inferred tree
# this is inclusive of all cases so it is a general file. you don't need to use GV_tree.py, you can use this one for all kinds of tree creations
...
orig_pos = pd.read_csv('sc_2000_final_df.tsv',sep='\t',header=None) # final data frame for finding the sites which are mutated
config.insert(loc = 0,column = 'site',value = orig_pos[1]) # putting the site information at the first column

cols = [i for i in range(0,2000)]
...
print(config2.columns)
cluster_labels = list(np.loadtxt('sc_2000_inferred_cluster_labels.txt',dtype='int')) # now finding the labels corresponding to each unique cell
columns = dict()
...
for cluster_no in range(no_of_clusters):
  if cluster_no not in columns.values():
    cell_instance = cluster_labels.index(cluster_no) # index at which the first instance of that cluster appears in the crowd of 2000 cells
    config2.insert(loc = no_of_clusters-missing,column = cell_instance,value = config[cell_instance])
...
with open('Phalcon_Inferred_tree_ado_dec_2000_data_'+str(data)+'.gv','w') as f:
  f.write('digraph G {\n')
  for node in tree_nw.traverse("preorder"):
