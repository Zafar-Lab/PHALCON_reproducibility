...
for aml_file_name in sub_folders:
    ...
    annovar_position = [28602235, 28602237, 28602238, 28602239, 28602240,  28602243, 28602245]

    phalcon_position = [28602234,28602235,28602236, 28602237, 28602238, 28602239, 28602240,28602242, 28602243, 28602244]

    for i in range(annovar_file_np.shape[0]):
        if 'FLT3' in annovar_file_np[i][1] and annovar_file_np[i][0]=='intronic':
            annovar_position.append(annovar_file_np[i][3])   # collect the genomic loci of such FLT3 intronic variants
            if str(annovar_file_np[i][3]) in phalcon_variants:
                phalcon_position.append(annovar_file_np[i][3])
            else:
                phalcon_position.append(annovar_file_np[i][3]-1)
    # if all those positions are within 10bp, remove that entire variant from the final list of variants
    remove_pos_annovar = []
    for pos in range(len(annovar_position)-1):
        if int(annovar_position[pos+1])-int(annovar_position[pos]) <10:
            remove_pos_annovar.extend([annovar_position[pos],annovar_position[pos+1]])
    
    remove_pos_phalcon = []
    for pos in range(len(phalcon_position)-1):
        if int(phalcon_position[pos+1])-int(phalcon_position[pos]) <10:
            remove_pos_phalcon.extend([phalcon_position[pos],phalcon_position[pos+1]])
