...
vcf_file_name = '/home/priya/Documents/priya/MPT post-processing/'+data+'/Spectral_prior_FINAL/'+data+'_orig_spectral_non_zero_outputInference.vcf'
...
with open('/home/priya/Documents/priya/MPT post-processing/'+data+'/Spectral_prior_FINAL/'+data+'_orig_spectral_dbsnp_non_zero_outputInference.vcf',"w") as vcfFile:
    pos = vcf_np.shape[0]
...
######################################################(clonal)
mpileup = np.loadtxt(data+'_clonal.mpileup',dtype='str')
print("mpileup shape :",mpileup.shape)
clonal_list = np.array(mpileup[:,1],dtype='str')
print("Clonal mutations list :",clonal_list)
vcf_obtained = np.loadtxt('/home/priya/Documents/priya/MPT post-processing/'+data+'/Spectral_prior_FINAL/'+data+'_orig_spectral_dbsnp_non_zero_outputInference.vcf',dtype='str')
print("vcf obtained :\n",vcf_obtained)
vcf_df = pd.DataFrame(vcf_obtained)
clonal_vcf = np.array(vcf_df[vcf_df[1].isin(clonal_list)],dtype='object')
print("VCF of only clonal sites :",clonal_vcf)
print("whether mpileup and vcf clonal are of same dimensions or not :", mpileup.shape[0]==clonal_vcf.shape[0])
...
tag='/home/priya/Documents/priya/MPT post-processing/'+data+'/Spectral_prior_FINAL/'+data
#lklhd_df = pd.read_csv(tag+'_orig_spectral_final_lklhds.tsv',sep='\t',header=None)
#final_df_rc = pd.read_csv(tag+'_orig_spectral_final_df.tsv',sep='\t',header=None)
#genotype = pd.read_csv(tag+'_orig_spectral_inferred_genotypes.tsv',sep=',',header=None)
...
with open(data+"_post_processed_clonal.vcf","w") as vcfFile:
    vcfFile.write("##fileformat=VCFv4.1\n")
    vcfFile.write("##source=OurAlgo" + "OurAlgo v" + '0' + "." + '1' + "." + '0' + "\n")
    vcfFile.write("##FILTER=<ID=LowQual,Description=\"Low quality\">\n")
    vcfFile.write("##INFO=<ID=DP,Number=1,Type=Integer,Description=\"Approximate read depth; some reads may have been filtered\">\n")
...
########################################################################## (non clonal)
mpileup = np.loadtxt(data+'_nonclonal.mpileup',dtype='str')
print("mpileup shape :",mpileup.shape)
nonclonal_list = np.array(mpileup[:,1],dtype='str')
print("Clonal mutations list :",nonclonal_list)
vcf_obtained = np.loadtxt('/home/priya/Documents/priya/MPT post-processing/'+data+'/Spectral_prior_FINAL/'+data+'_orig_spectral_dbsnp_non_zero_outputInference.vcf',dtype='str')
print("vcf obtained :\n",vcf_obtained)
vcf_df = pd.DataFrame(vcf_obtained)
nonclonal_vcf = np.array(vcf_df[vcf_df[1].isin(nonclonal_list)],dtype='object')
print("VCF of only clonal sites :",nonclonal_vcf)
print("whether mpileup and vcf clonal are of same dimensions or not :", mpileup.shape[0]==nonclonal_vcf.shape[0])

sites_to_be_removed = []
for i in range(nonclonal_vcf.shape[0]):
  if nonclonal_vcf[i][1] == mpileup[i][1]:
    site = nonclonal_vcf[i][1]
    pos = nonclonal_vcf[i][1]
    ref = nonclonal_vcf[i][3]
    alt = nonclonal_vcf[i][4]
    seq = mpileup[i][4]
    ref_count = seq.upper().count(ref.upper())
    alt_count = seq.upper().count(alt.upper())
    if ref_count == 0:
      ...
      if alt_count/ref_count > 0.1:
        sites_to_be_removed.append(site)
print("Sites being removed :",sites_to_be_removed)
nonclonal_df = pd.DataFrame(nonclonal_vcf)
post_process_vcf = np.array(nonclonal_df[~nonclonal_df[1].isin(sites_to_be_removed)],dtype='object')
print("Final vcf shape :",post_process_vcf.shape)

numCells = post_process_vcf.shape[1]-9
with open(data+"_post_processed_nonclonal.vcf","w") as vcfFile:
    vcfFile.write("##fileformat=VCFv4.1\n")
    vcfFile.write("##source=OurAlgo" + "OurAlgo v" + '0' + "." + '1' + "." + '0' + "\n")
    vcfFile.write("##FILTER=<ID=LowQual,Description=\"Low quality\">\n")
    vcfFile.write("##INFO=<ID=DP,Number=1,Type=Integer,Description=\"Approximate read depth; some reads may have been filtered\">\n")
    ...
    for i in range(pos):
        ...
        vcfFile.write(post_process_vcf[i][8]+'\t')
        for cell in range(numCells):
          vcfFile.write(post_process_vcf[i][cell]+'\t')
        vcfFile.write('\n')
