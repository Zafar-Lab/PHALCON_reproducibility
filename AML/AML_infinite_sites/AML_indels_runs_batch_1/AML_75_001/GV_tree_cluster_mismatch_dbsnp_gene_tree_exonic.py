...
dbsnp_non_zero_avinput_file_name = '/home/priya/Downloads/Final_Stage/AML_Results/AML_indels_runs_batch_1/'+data+'/'+data+'_indels_dbsnp_non_zero.avinput.variant_function'
...
with open(data+'_indels_dbsnp_nz_inferred_tree_exonic.gv','w') as f:
  f.write('digraph G {\n')
  for node in tree_nw.traverse("preorder"):
...
with open(data+'_indels_dbsnp_nz_nonempty_inferred_tree_exonic.gv','w') as f:
  f.write('digraph G {\n')
  for node in tree_nw.traverse("preorder"):
    list_of_label = node.label
    delimiter = ','
...
def gene_tree_non_empty(data): # this function will convert all mutations named as "1:29997234" into their gene names
  ...
  with open(data+'_indels_dbsnp_nz_nonempty_inferred_tree_exonic.gv','r') as f:
     content = f.readlines()
  ...
  with open(data+'_indels_dbsnp_nz_nonempty_gene_tree_exonic.gv','w') as f:
    for i in range(len(content)):
      f.write(content[i])
  print("nw tree written as gv format")

  s = Source.from_file(data+'_indels_dbsnp_nz_nonempty_gene_tree_exonic.gv')
  s.render()


def gene_tree(data): # this function will convert all mutations named as "1:29997234" into their gene names
  gene_name = pd.read_csv(data+'_indels_dbsnp_non_zero.avinput.variant_function',sep='\t',header=None)
  with open(data+'_indels_dbsnp_nz_inferred_tree_exonic.gv','r') as f:
     content = f.readlines()
  gene_name = np.array(gene_name,dtype='object')
  gene_pos = dict()
  for i in range(gene_name.shape[0]):
  ...
  with open(data+'_indels_dbsnp_nz_gene_tree_exonic.gv','w') as f:
    for i in range(len(content)):
      f.write(content[i])
  print("nw tree written as gv format")

  s = Source.from_file(data+'_indels_dbsnp_nz_gene_tree_exonic.gv')
  s.render()
