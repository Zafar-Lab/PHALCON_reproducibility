...
for i in range(int(m_pos)):   # at remaining positions putting back, parallel and heterozygous bases on some probability
  if random.random() < 0.3:  # 30% clonal : out of that, 90% clonal and 10% back at clonal
    if random.random() < 0.1:
      ...
      TG_mut[i] = list(np.ones((1,numCells))[0]) # Putting clonal mutations at 90% of the sites
      TG_mut_list = copy.copy(TG_mut[i])
