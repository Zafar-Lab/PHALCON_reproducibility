...
manual_oncoplot.to_csv("Dataframe_for_manual_oncoplot_creation_AML_gene_wise.tsv",sep='\t',header=True,index=True)


df = pd.read_csv("Dataframe_for_manual_oncoplot_creation_AML_gene_wise.tsv",sep='\t',index_col=[0])
df[df > 0] = 1
...
print("Variants which are not intronic :\n",df_all_variants)
df_all_variants = df_all_variants.drop(df_all_variants[df_all_variants['Sample_no'].str.contains('PDX')].index)
df_all_variants = df_all_variants.drop(df_all_variants[~df_all_variants['Sample_no'].str.endswith('_001')].index)
print("PDX samples dropped ...")
print(df_all_variants)
df_all_variants.to_csv('All non intronic variants (intronic, ncRNA_intronic, intergenic removed) non pdx samples aml',sep='\t',index=False)
...
manual_oncoplot.to_csv("Dataframe_for_manual_oncoplot_creation_AML_gene_wise_without_PDX_001_only.tsv",sep='\t',header=True,index=True)


df = pd.read_csv("Dataframe_for_manual_oncoplot_creation_AML_gene_wise_without_PDX_001_only.tsv",sep='\t',index_col=[0])
df[df > 0] = 1
no_of_samples = df.shape[1]
print(df)
df['sum'] = df.sum(axis=1)
df = df.sort_values('sum',ascending=False)
df.drop(df.columns[range(0,no_of_samples)], axis=1, inplace=True)
df.to_csv("Gene_wise_AML_no_of_samples_affected_without_PDX_001_only.tsv")
