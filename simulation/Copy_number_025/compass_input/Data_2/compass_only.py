...
for i in range(len(new_content)):
  ...
  line4 = re.sub("\\n",'',new_content[i])
  new_content[i] = line4
  line5 = re.sub("<B>CNLOH RegionREF,ALT </B>",'', new_content[i])
  new_content[i] = line5
  line6 = re.sub("<B>CNLOH RegionALT,ALT </B>,<B>CNLOH Region12:ALT,ALT,REF,REF </B>,<B>CNLOH Region13:ALT,REF </B>",'',new_content[i])
  new_content[i] = line6
  line7 = re.sub("<B>CNLOH Region12:ALT,ALT,ALT,ALT </B>",'',new_content[i])
  new_content[i] = line7
...
for i in new_content:
  print(i)
  if i==' ' or '' or not i:  # if there is no label, some instances empty string might also come into picture
    continue
  else:
    sites = i.split(',')
