...
argParser.add_argument('-i', '--inReadCountFileName', type=str, default = '/home/priya/Downloads/Final_Stage/Real Data Results/TN1/readCounts_MBTN1_nodups.tsv')
argParser.add_argument('-g', '--inQualityFileName', type=str,default = '/home/priya/Downloads/Final_Stage/Real Data Results/TN1/gq_file_TN1.tsv')
argParser.add_argument('-o', '--outputPrefix', type=str,default='TN1_orig_hierarchical_bf_test_')
...
argParser.add_argument('-s', '--seed', type=int, default = 12091)
...
print(df)
list_bulk_evidence_not_found = [127379885,33762586,57398186,54647271,20208852]
df = df[df[1].isin([54647271])]
print(df)
df = df.to_numpy()
...
if isQualityFilteringOn: 
    ...
    df_gq = df_gq[df_gq[0].isin([54647271])]
...
pd.DataFrame(data_df).to_csv(outputPrefixName+'final_phalcon_lklhds.tsv',sep='\t',header=False,index=False)
graph_laplacian = generate_graph_laplacian(df=data_df, nn=20)


log_df = pd.DataFrame(data_df).applymap(lambda x: np.log(x))
pd.DataFrame(log_df).to_csv(outputPrefixName+'final_phalcon_log_lklhds.tsv',sep='\t',header=False,index=False)
...
Labelfile = '/home/priya/Downloads/Final_Stage/Real Data Results/TN1/Hierarchical_prior_FINAL/Statistical_test/TN1_orig_hierarchical_inferred_cluster_labels_modified.txt'   # True cell to cluster map

final_labels = list(np.loadtxt(Labelfile))
print(final_labels)
...
print("Best likelihoods: ", lklhd_s)
