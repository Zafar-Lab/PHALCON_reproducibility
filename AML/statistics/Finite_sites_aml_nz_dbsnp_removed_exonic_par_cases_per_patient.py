...
for patient in sorted(os.listdir(BASE_DIR)):
    ...
    try:
        ...
        # Keep only par
        # -----------------------------
        par_df = merged[merged["config"] == "Par"].copy()
        ...
        if par_df.empty:
            ...
            print(f"[OK] {patient}: exonic par = 0")
            continue

        # -----------------------------
        # Build intervals
        # -----------------------------
        par_df = make_interval(par_df, "chr", "site", "ref", "alt")
        ...
        summary.append({
            "patient": patient,
            "num_exonic_par": exonic_par.shape[0]
        })

        if not exonic_par.empty:
            all_exonic_par.append(exonic_par)

        print(f"[OK] {patient}: exonic par = {exonic_par.shape[0]}")
...
final_exonic_par_df = (
    pd.concat(all_exonic_par, ignore_index=True)
    if all_exonic_par
    else pd.DataFrame(
        columns=["chr", "site", "ref", "alt", "gene", "patient"]
    )
...
summary_df.to_csv(
    os.path.join(BASE_DIR, "exonic_par_summary.tsv"),
    sep="\t", index=False
)

final_exonic_par_df.to_csv(
    os.path.join(BASE_DIR, "exonic_par_variants_all_patients_AML_finite_sites.tsv"),
    sep="\t", index=False
)
frames.append(final_exonic_par_df)
print("\nDONE.")
...
pd.concat(frames).to_csv("all_exonic_par_cases_aml_finite_sites_nz_dbsnp.tsv",sep='\t',index=False)
